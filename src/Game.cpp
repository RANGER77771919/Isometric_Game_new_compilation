...
void Game::updateChunks() {
    ...
    // Convertir a coordenadas de chunk usando floor (IMPORTANTE: floor para negativos)
    BlockPos camBlockPos(
        static_cast<int>(std::floor(camX)),
        static_cast<int>(std::floor(camY)),
        static_cast<int>(std::floor(camZ))
    );
    ChunkPos currentChunkPos = camBlockPos.toChunkPos();
...
void Game::render() {
    ...
    // Convertir a coordenadas de chunk usando floor
    BlockPos camBlockPos(
        static_cast<int>(std::floor(camX)),
        static_cast<int>(std::floor(camY)),
        static_cast<int>(std::floor(camZ))
    );
    ChunkPos camChunkPos = camBlockPos.toChunkPos();

    // Obtener chunks visibles para renderizado
    std::vector<Chunk*> visibleChunks = m_world->getChunksAround(camChunkPos, RENDER_RADIUS);

    // Renderizar mundo
    m_renderer->renderWorld(visibleChunks, *m_camera);

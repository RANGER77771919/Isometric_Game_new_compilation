...
void Renderer::renderBlock(float screenX, float screenY, BlockType type, int worldY, float zoom) {
    ...
    // Obtener textura según tipo de bloque (switch original)
    std::string textureName;
    switch (type) {
        case BlockType::PASTO:
            textureName = "grass";
            break;
        case BlockType::TIERRA:
            textureName = "dirt";
            break;
        case BlockType::PIEDRA:
            textureName = "stone";
            break;
        case BlockType::ARENA:
            textureName = "sand";
            break;
        case BlockType::NIEVE:
            textureName = "snow";
            break;
        case BlockType::AGUA:
            textureName = "water";
            break;
        case BlockType::HIERBA_SANGRE:
            textureName = "blood_grass";
            break;
        case BlockType::PASTO_FULL:
            textureName = "full_blood_grass";
            break;
        default:
            return;  // No renderizar
    }

    SDL_Texture* texture = m_textureManager.getTexture(textureName);
    if (!texture) {
        return;
    }
    ...
    // Aplicar zoom al tamaño del tile
...
bool Renderer::isChunkVisible(const Chunk* chunk, const Camera& camera, int screenWidth, int screenHeight) {
    ...
    float worldXStart = static_cast<float>(pos.x * BlockConfig::CHUNK_SIZE);
    float worldZStart = static_cast<float>(pos.z * BlockConfig::CHUNK_SIZE);
    float worldXEnd = worldXStart + BlockConfig::CHUNK_SIZE;
    float worldZEnd = worldZStart + BlockConfig::CHUNK_SIZE;

    // Proyectar las 4 esquinas del bounding box (Y=0 y Y=maxHeight)
    // REVERTIDO: Y crece hacia ARRIBA, necesitamos ambos extremos
    float minX = FLT_MAX, maxX = -FLT_MAX;
    float minY = FLT_MAX, maxY = -FLT_MAX;

    for (float y : {0.0f, 110.0f}) {  // Y=0 (suelo) y Y=110 (altura máxima típica)
        for (float x : {worldXStart, worldXEnd}) {
            for (float z : {worldZStart, worldZEnd}) {
                float sx, sy;
                camera.worldToScreen(x, y, z, sx, sy);
    ...
    return !(maxX < -margin || minX > screenWidth + margin ||
             maxY < -margin || minY > screenHeight + margin);
}

void Renderer::renderWorld(const std::vector<Chunk*>& chunks, const Camera& camera) {
    // Reutilizar cache de tiles (clear no libera memoria)
    m_tileCache.clear();

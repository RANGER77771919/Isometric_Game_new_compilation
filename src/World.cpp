...
World::World(uint32_t seed)
    : m_seed(seed)
    , m_rng(seed)
    , m_biomeCacheInitialized(false)  // Inicializar flag de cache
...
void World::generateTerrain(Chunk* chunk) {
    ...
    // OPTIMIZACIÓN 2: Pre-calcular alturas del terreno primero
    int terrainHeights[BlockConfig::CHUNK_SIZE][BlockConfig::CHUNK_SIZE];

    for (int x = 0; x < BlockConfig::CHUNK_SIZE; x++) {
        for (int z = 0; z < BlockConfig::CHUNK_SIZE; z++) {
            float noiseValue = terrainNoise[x][z];
            terrainHeights[x][z] = static_cast<int>((noiseValue + 1.0f) * 0.5f * 60.0f) + 50;
        }
    }

    // OPTIMIZACIÓN 3: Pre-calcular ruido 3D de cuevas para rangos necesarios
    // Array 3D compacto: [x][z][y] solo para rango de cuevas [10, maxTerrainHeight-5]
    ...
    // Rango de cuevas: Y=[10, maxTerrainHeight-5]
    const int caveYStart = 10;
    const int caveYEnd = maxTerrainHeight - 5;
    const int caveYRange = caveYEnd - caveYStart + 1;

    // Array 3D para cache de cuevas [16][16][caveYRange]
    // Usamos vector dinámico para no desperdiciar memoria estática
    std::vector<std::vector<std::vector<float>>> caveNoise(
        BlockConfig::CHUNK_SIZE,
        std::vector<std::vector<float>>(
            BlockConfig::CHUNK_SIZE,
            std::vector<float>(caveYRange)
        )
    );
    ...
    for (int x = 0; x < BlockConfig::CHUNK_SIZE; x++) {
        for (int z = 0; z < BlockConfig::CHUNK_SIZE; z++) {
            ...
            for (int y = caveYStart; y <= caveYEnd; y++) {
                int caveIndex = y - caveYStart;
                caveNoise[x][z][caveIndex] = m_noiseCaves.GetNoise(
        ...
        for (int z = 0; z < BlockConfig::CHUNK_SIZE; z++) {
            ...
            for (int y = 0; y < BlockConfig::WORLD_HEIGHT; y++) {
                ...
                if (y == 0) {
                    ...
                    // OPTIMIZACIÓN: Usar noise de cuevas cacheado
                    if (y > caveYStart && y < caveYEnd) {
                        int caveIndex = y - caveYStart;
                        if (caveNoise[x][z][caveIndex] > 0.4f) {
...
BlockType World::getBiomeFromNoise(float biomeValue) const {
    // OPTIMIZACIÓN: Inicializar cache la primera vez (lazy initialization)
    if (!m_biomeCacheInitialized) {
        // Precalcular todos los biomas posibles
        ...
        m_biomeCacheInitialized = true;
    }

    // OPTIMIZACIÓN: Usar cache en lugar de if-else chain
...
int World::getTerrainHeight(int x, int z) const {
    ...
    return static_cast<int>((noiseValue + 1.0f) * 0.5f * 60.0f) + 50;
}
